...
export default function ProductDashboard({ standalone }) {
  const [products, setProducts] = useState([]);
  const [searchText, setSearchText] = useState('');
  const [status, setStatus] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [form, setForm] = useState({ Name: '', price: '', quantity: '', status: 'ordered', description: '' });
  ...
  const resetForm = () => { setForm({ Name: '', price: '', quantity: '', status: 'ordered', description: '' }); setEditId(null); };
  ...
  const handleEdit = (product) => {
    setForm({ Name: product.Name || product.name, price: product.price, quantity: product.quantity, status: product.status || 'ordered', description: product.description || '' });
      ...
      <div className="container">
        <div className="stats">
          ...
          <div className="search">
            <input type="text" placeholder="Search by name or description..." value={searchText} onChange={(e) => setSearchText(e.target.value)} />
                  ...
                  <tbody>
                    {products.map(p => (
                      <tr key={p.id}>
                        ...
                        <td className="price-cell">${Number(p.price || 0).toFixed(2)}</td>
                        <td>
